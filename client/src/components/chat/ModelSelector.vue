...
<template>
  <div style="position:relative">
    ...
    <Transition name="dropdown">
      <div v-if="show" :style="{ position: 'absolute', top: '100%', left: 0, marginTop: '4px', width: '224px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--background)', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 50, overflow: 'hidden', transformOrigin: 'top left' }">
        <div style="padding:4px 0">
          <div v-for="m in MODELS" :key="m.id" class="model-item" :style="{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', cursor: 'pointer', fontSize: '14px', color: 'var(--text-primary)', background: chatStore.config.model === m.id ? 'var(--input-bg)' : 'transparent', transition: 'background 0.15s' }" @click="select(m.id)" @mouseenter="($event.currentTarget as HTMLElement).style.background = 'var(--input-hover)'" @mouseleave="($event.currentTarget as HTMLElement).style.background = chatStore.config.model === m.id ? 'var(--input-bg)' : 'transparent'">
            <div>
