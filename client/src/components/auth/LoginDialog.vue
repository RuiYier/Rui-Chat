<script setup lang="ts">
import { ref, reactive } from 'vue'
import { useAuthStore } from '@/stores/auth'
import { ElMessage } from 'element-plus'

const props = defineProps<{ modelValue: boolean }>()
const emit = defineEmits<{
  'update:modelValue': [value: boolean]
  success: []
}>()

const authStore = useAuthStore()

// View state: 'oauth' | 'email-login' | 'email-register'
const view = ref<'oauth' | 'email-login' | 'email-register'>('oauth')
const loading = ref(false)
const loginForm = reactive({ username: '', password: '' })
const registerForm = reactive({ username: '', password: '', confirmPassword: '', email: '' })

function goToGoogle() {
  window.location.href = '/api/auth/google'
}

function goToGithub() {
  window.location.href = '/api/auth/github'
}

function resetState() {
  view.value = 'oauth'
  loginForm.username = ''
  loginForm.password = ''
  registerForm.username = ''
  registerForm.password = ''
  registerForm.confirmPassword = ''
  registerForm.email = ''
  loading.value = false
}

function handleClose() {
  resetState()
  emit('update:modelValue', false)
}

async function handleLogin() {
  if (!loginForm.username || !loginForm.password) {
    ElMessage.warning('请填写用户名和密码')
    return
  }
  loading.value = true
  try {
    await authStore.login(loginForm.username, loginForm.password)
    ElMessage.success('登录成功')
    resetState()
    emit('success')
  } catch (err: any) {
    ElMessage.error(err.response?.data?.message || '登录失败')
  } finally {
    loading.value = false
  }
}

async function handleRegister() {
  if (!registerForm.username || !registerForm.password) {
    ElMessage.warning('请填写用户名和密码')
    return
  }
  if (registerForm.password !== registerForm.confirmPassword) {
    ElMessage.warning('两次密码不一致')
    return
  }
  if (registerForm.password.length < 6) {
    ElMessage.warning('密码至少 6 位')
    return
  }
  loading.value = true
  try {
    await authStore.register(registerForm.username, registerForm.password, registerForm.email || undefined)
    // 注册成功后自动登录
    await authStore.login(registerForm.username, registerForm.password)
    ElMessage.success('注册成功')
    resetState()
    emit('success')
  } catch (err: any) {
    ElMessage.error(err.response?.data?.message || '注册失败')
  } finally {
    loading.value = false
  }
}
</script>

<template>
  <el-dialog
    :model-value="modelValue"
    @update:model-value="(val: boolean) => val ? null : handleClose()"
    width="420px"
    :close-on-click-modal="false"
    :show-close="true"
    @open="resetState"
  >
    <!-- ====== OAuth2 视图 ====== -->
    <template v-if="view === 'oauth'">
      <div :style="{ textAlign: 'center', marginBottom: '24px' }">
        <h2 :style="{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--text-primary)' }">Rui Chat</h2>
        <p :style="{ fontSize: '0.875rem', marginTop: '8px', color: 'var(--text-secondary)' }">登录以开始对话</p>
      </div>

      <!-- Google -->
      <button
        :style="{
          width: '100%',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          gap: '12px',
          padding: '10px 16px',
          borderRadius: '8px',
          fontSize: '0.875rem',
          fontWeight: '500',
          border: '1px solid #D1D5DB',
          background: '#fff',
          color: '#374151',
          cursor: 'pointer',
          marginBottom: '12px',
          transition: 'background 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.background = '#F9FAFB'"
        @mouseleave="($event.target as HTMLElement).style.background = '#fff'"
        @click="goToGoogle"
      >
        <svg width="20" height="20" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
        使用 Google 登录
      </button>

      <!-- GitHub -->
      <button
        :style="{
          width: '100%',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          gap: '12px',
          padding: '10px 16px',
          borderRadius: '8px',
          fontSize: '0.875rem',
          fontWeight: '500',
          border: 'none',
          background: '#24292F',
          color: '#fff',
          cursor: 'pointer',
          marginBottom: '24px',
          transition: 'background 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.background = '#2d333b'"
        @mouseleave="($event.target as HTMLElement).style.background = '#24292F'"
        @click="goToGithub"
      >
        <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
        使用 GitHub 登录
      </button>

      <!-- Divider -->
      <div :style="{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }">
        <div :style="{ flex: '1', height: '1px', background: 'var(--border)' }"></div>
        <span :style="{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }">或</span>
        <div :style="{ flex: '1', height: '1px', background: 'var(--border)' }"></div>
      </div>

      <!-- 切换到邮箱登录 -->
      <button
        :style="{
          width: '100%',
          padding: '10px 16px',
          borderRadius: '8px',
          fontSize: '0.875rem',
          fontWeight: '500',
          border: '1px solid var(--border)',
          background: 'var(--background)',
          color: 'var(--text-primary)',
          cursor: 'pointer',
          transition: 'background 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.background = 'var(--input-bg)'"
        @mouseleave="($event.target as HTMLElement).style.background = 'var(--background)'"
        @click="view = 'email-login'"
      >
        使用邮箱登录
      </button>
    </template>

    <!-- ====== 邮箱登录视图 ====== -->
    <template v-else-if="view === 'email-login'">
      <div :style="{ textAlign: 'center', marginBottom: '24px' }">
        <h2 :style="{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--text-primary)' }">邮箱登录</h2>
        <p :style="{ fontSize: '0.875rem', marginTop: '8px', color: 'var(--text-secondary)' }">使用用户名和密码登录</p>
      </div>

      <el-form @submit.prevent="handleLogin" label-position="top">
        <el-form-item label="用户名">
          <el-input v-model="loginForm.username" placeholder="用户名或邮箱" size="large" />
        </el-form-item>
        <el-form-item label="密码">
          <el-input v-model="loginForm.password" type="password" placeholder="密码" show-password size="large" />
        </el-form-item>
      </el-form>

      <button
        :style="{
          width: '100%',
          padding: '10px 16px',
          borderRadius: '8px',
          fontSize: '0.875rem',
          fontWeight: '500',
          border: 'none',
          background: 'var(--button-primary-bg)',
          color: '#fff',
          cursor: 'pointer',
          marginTop: '8px',
          transition: 'opacity 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.opacity = '0.9'"
        @mouseleave="($event.target as HTMLElement).style.opacity = '1'"
        @click="handleLogin"
      >
        {{ loading ? '登录中...' : '登录' }}
      </button>

      <div :style="{ textAlign: 'center', marginTop: '16px' }">
        <span :style="{ fontSize: '0.875rem', color: 'var(--text-tertiary)' }">没有账号？</span>
        <button
          :style="{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--ring)', background: 'none', border: 'none', cursor: 'pointer' }"
          @click="view = 'email-register'"
        >立即注册</button>
      </div>

      <button
        :style="{
          width: '100%',
          padding: '8px 16px',
          borderRadius: '8px',
          fontSize: '0.8rem',
          border: '1px solid var(--border)',
          background: 'var(--background)',
          color: 'var(--text-secondary)',
          cursor: 'pointer',
          marginTop: '12px',
          transition: 'background 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.background = 'var(--input-bg)'"
        @mouseleave="($event.target as HTMLElement).style.background = 'var(--background)'"
        @click="view = 'oauth'"
      >
        ← 返回其他登录方式
      </button>
    </template>

    <!-- ====== 邮箱注册视图 ====== -->
    <template v-else>
      <div :style="{ textAlign: 'center', marginBottom: '24px' }">
        <h2 :style="{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--text-primary)' }">创建账号</h2>
        <p :style="{ fontSize: '0.875rem', marginTop: '8px', color: 'var(--text-secondary)' }">注册后自动登录</p>
      </div>

      <el-form @submit.prevent="handleRegister" label-position="top">
        <el-form-item label="用户名">
          <el-input v-model="registerForm.username" placeholder="用户名" size="large" />
        </el-form-item>
        <el-form-item label="密码">
          <el-input v-model="registerForm.password" type="password" placeholder="至少 6 位" show-password size="large" />
        </el-form-item>
        <el-form-item label="确认密码">
          <el-input v-model="registerForm.confirmPassword" type="password" placeholder="再次输入密码" show-password size="large" />
        </el-form-item>
        <el-form-item label="邮箱 (可选)">
          <el-input v-model="registerForm.email" placeholder="邮箱" size="large" />
        </el-form-item>
      </el-form>

      <button
        :style="{
          width: '100%',
          padding: '10px 16px',
          borderRadius: '8px',
          fontSize: '0.875rem',
          fontWeight: '500',
          border: 'none',
          background: 'var(--button-primary-bg)',
          color: '#fff',
          cursor: 'pointer',
          marginTop: '8px',
          transition: 'opacity 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.opacity = '0.9'"
        @mouseleave="($event.target as HTMLElement).style.opacity = '1'"
        @click="handleRegister"
      >
        {{ loading ? '注册中...' : '注册' }}
      </button>

      <div :style="{ textAlign: 'center', marginTop: '16px' }">
        <span :style="{ fontSize: '0.875rem', color: 'var(--text-tertiary)' }">已有账号？</span>
        <button
          :style="{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--ring)', background: 'none', border: 'none', cursor: 'pointer' }"
          @click="view = 'email-login'"
        >去登录</button>
      </div>

      <button
        :style="{
          width: '100%',
          padding: '8px 16px',
          borderRadius: '8px',
          fontSize: '0.8rem',
          border: '1px solid var(--border)',
          background: 'var(--background)',
          color: 'var(--text-secondary)',
          cursor: 'pointer',
          marginTop: '12px',
          transition: 'background 0.2s',
        }"
        @mouseenter="($event.target as HTMLElement).style.background = 'var(--input-bg)'"
        @mouseleave="($event.target as HTMLElement).style.background = 'var(--background)'"
        @click="view = 'oauth'"
      >
        ← 返回其他登录方式
      </button>
    </template>
  </el-dialog>
</template>
